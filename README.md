...
```
.package(url:"https://github.com/wildberries-tech/wba_analytics_sdk_ios.git",exact:  "4.0.4")
```
...
- **enableAutomaticEvents:** Включите (`true`) для автоматического отслеживания установок, запусков приложения и времени активности. Если ваш продукт — библиотека для интеграции, держите `false`. Автоматические события для интегрируемых продуктов работают нестабильно. Необязательный параметр, по умолчанию `false`. [Подробнее](#-системные-события)
...
Все события ниже управляются одним флагом `enableAutomaticEvents` в инициализаторе ресивера.
**По умолчанию он выключен (`false`) — автоматические события не отправляются.**
...
> **Включите `enableAutomaticEvents: true` для автоматического отслеживания установок,
> запусков приложения и времени активности.**
>
> ⚠️ **Если ваш продукт — библиотека для интеграции, держите `false`.**
> Автоматические события для интегрируемых продуктов работают нестабильно.

Флаг не влияет на `user_engagement`, на события с явным вызовом (`trackEvent`, `trackLaunchURL`)
и на отправку батчей — они работают одинаково при любом значении.

Значение флага уходит в `meta` каждого батча под ключом `enable_automatic_events`
(`true` / `false`) — по нему на стороне бэкенда видно, включён ли сбор у интегратора.
