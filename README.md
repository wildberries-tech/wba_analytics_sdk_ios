...
- **enableAutomaticEvents:** Отправка автоматических событий SDK. Необязательный параметр, по умолчанию `true`. Отключает отправку автоматических событий: если ваш проект — SDK, который интегрируется в другой продукт, нужно поставить `enableAutomaticEvents: false`. [Подробнее](#-системные-события)
...
Все события ниже управляются одним флагом `enableAutomaticEvents` (по умолчанию `true`).
...
> ⚠️ **Если ваш проект — SDK, который интегрируется в другой продукт, нужно поставить
> `enableAutomaticEvents: false`.** Тогда SDK перестаёт отправлять все автоматические события из
> таблицы выше. Эти события описывают жизненный цикл приложения, а встроенный SDK им не управляет:
> в чужом приложении они не описывают ничью сессию и только удваивают трафик событий у хоста.
