...
- у `application_start` исправлена доставка, добавлен параметр `processor_name`, а `cpu` теперь округляется до двух знаков после запятой;
...
### Включите (`true`) для автоматического отслеживания установок, запусков приложения и времени активности
...
### Если ваш продукт — библиотека для интеграции

> ⚠️ **Держите `false`.** Автоматические события для интегрируемых продуктов работают нестабильно.
...
- Параметр `cpu` (тактовая частота, ГГц) округляется до двух знаков после запятой: раньше у части
  старых процессоров уходило три знака (`0.533`, `0.412`), теперь — `0.53`, `0.41`.
...
- [ ] Решили, нужны ли автоматические события: нужны — `enableAutomaticEvents: true`, продукт является библиотекой для интеграции — держим `false`
